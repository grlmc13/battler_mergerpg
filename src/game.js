...
class GameScene extends Phaser.Scene {
    ...
    generateRegularWave(waveNumber, difficultyMultiplier) {
        ...
        let placedEnemies = 0;
        for (let i = 0; i < availablePositions.length && placedEnemies < numEnemies; i++) {
            const pos = availablePositions[i];
            const unitType = Phaser.Utils.Array.GetRandom(unitTypes);
            const unitData = window.gameConfig.UNIT_TYPES[unitType];
            
            // Проверяем, можно ли разместить юнит в этой позиции
            if (this.gridSystem.canPlaceEnemyUnit(pos.x, pos.y, unitData.size)) {
                const unit = this.createEnemyUnit(unitType, pos.x, pos.y);
                if (unit) {
                    ...
                    console.log(`Размещен враг ${unitType} в позиции (${pos.x}, ${pos.y})`);
                }
            }
        }
        
        console.log(`Создано ${placedEnemies} врагов для волны ${waveNumber}`);
    }
